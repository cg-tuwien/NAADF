...
RWStructuredBuffer<uint2> taaDistMinMax;
...
void reprojectOldSamples(uint3 globalID : SV_DispatchThreadID, uint3 groupID : SV_GroupID, uint localIndex : SV_GroupIndex)
{
    ...
    taaDistMinMax[globalID.x] = uint2(f32tof16(distMinMax.x) | (f32tof16(distMinMax.y) << 16), validNormalsSpec);
    ...
    taaSampleAccum[globalID.x] = uint2(f32tof16(colorSum.w) | (f32tof16(colorSum.r) << 16), f32tof16(colorSum.g) | (f32tof16(colorSum.b) << 16));
}

[numthreads(64, 1, 1)]
void calcNewTaaSample(uint3 globalID : SV_DispatchThreadID)
{
    int3 camPosInt = int3(camPosIntX, camPosIntY, camPosIntZ);
    if (globalID.x >= screenWidth * screenHeight)
        return;
    ...
    uint4 firstHit = firstHitData[globalID.x];
    FirstHitResult firstHitResult = getHitDataFromPlanes(entityInstancesHistory, taaIndex, firstHit, camPosInt, camPosFrac, rayDir);
    
    uint voxelType = firstHit.z & 0x7FFF;
    VoxelType firstHitVoxelTypeData = decompressVoxelType(voxelTypeData[voxelType]);
    uint specularNormals = getSpecularNormals(firstHit);
    
    uint2 lightComp = finalColor[globalID.x];
    float3 light = float3(f16tof32(lightComp.x & 0xFFFF), f16tof32(lightComp.x >> 16), f16tof32(lightComp.y));
    uint extraData8 = 0;
    bool isDiffuse = firstHit.y & 0x1;
    if (!isDiffuse) // Not diffuse
        extraData8 = 1 + pow(firstHitVoxelTypeData.roughness, 0.5f) * 30.5f;
    
    uint2 sampleComp = compressSample(voxelType == 0 ? f32tof16(65520) : (firstHit.w & 0x7FFF), light, firstHitResult.normalTang & 0x7, isDiffuse, specularNormals, extraData8, firstHit.x & 0x3FFF);
    ...
    newColorComp.x = f32tof16(sampleWeight + 1) | (f32tof16(taaColor.r) << 16);
...
technique ComputeCalc
{
    ...
    pass CalcNewTaaSample
    {
        ComputeShader = compile cs_5_0 calcNewTaaSample();
    }
};
