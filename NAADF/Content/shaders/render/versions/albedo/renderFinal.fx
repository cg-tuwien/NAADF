...
float4 MainPS(VertexShaderOutput input) : SV_Target
{
    ...
    float3 curColor = float3(f16tof32(colSamples.x >> 16), f16tof32(colSamples.y & 0xFFFF), f16tof32(colSamples.y >> 16)) / max(1, weight);
    
    if (showRayStep)
    {
        uint raySteps = colSamples.x;
    ...
    float3 tv = curColor / (1.0f + curColor);
