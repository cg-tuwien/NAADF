...
uint4 compressFirstHitData(float dist, uint4 normTangs, uint voxelTypeRaw, uint isDiffuse)
{
    uint4 firstHit;
    firstHit.x = ENTITY_FREE | (normTangs.x << 15);
...
void calcFirstHit(uint3 globalID : SV_DispatchThreadID)
{
    ...
    if (isVolumeHit)
    {
        ...
        for (i = 0; i < 4; ++i)
        {
            bool isHit = shootRay(curPosInt, curPosFrac, rayDir, 2000, rayResult);
            normTangs[i] = rayResult.normalComp;

            if (!isHit)
            {
                applyAtmosphere(oldPos, rayDir, absorption, light);
                break;
            }
            ...
            if (isAtmosphereInteraction)
                addLightForDirection(oldPos, rayDir, distance(curPosInt + curPosFrac, oldPos), absorption, light, true, 3, 3);
    ...
    firstHitData[globalID.x] = compressFirstHitData(distanceRay, normTangs, showRayStep ? rayResult.stepCount : voxelTypeRaw, isDiffuse);
