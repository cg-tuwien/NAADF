...
namespace Voxels
{
    ...
    public class MagicaVoxel {
        ...
        public Material[] Materials { get; private set; } = new Material[256];
                    ...
                    case MATL:
                        {
                            ...
                            var type = materialAttributes.GetValueOrDefault("_type");

                            if (type == "_emit")
                            {

                                material.emit = float.Parse(materialAttributes.GetValueOrDefault("_emit") ?? "0", CultureInfo.InvariantCulture);
                                material.flux = float.Parse(materialAttributes.GetValueOrDefault("_flux") ?? "0", CultureInfo.InvariantCulture);
                            }
                            if (type == "_metal")
                            {
                                material.metalic = float.Parse(materialAttributes.GetValueOrDefault("_metal") ?? "0", CultureInfo.InvariantCulture);
                                material.roughness = float.Parse(materialAttributes.GetValueOrDefault("_rough") ?? "0", CultureInfo.InvariantCulture);
                                material.ior = float.Parse(materialAttributes.GetValueOrDefault("_ior") ?? "0", CultureInfo.InvariantCulture);
                            }
        ...
        public VoxelDataBytes Flatten(BoundsXYZ worldBounds, int frame = 0) {
            if (Nodes.Count > 0) {
                var voxelData = new VoxelDataBytes(worldBounds.Size, Palette, Materials);
